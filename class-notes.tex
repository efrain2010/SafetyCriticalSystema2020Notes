...
\begin{itemize}
    \item What is a \underline{non-functional} requirement?
    \begin{itemize}
        \item Requirements that can't be tested in certain way
        \item Constraints on the system design, that may arise from the user requirements, technical disciplines or the external environment.
    \end{itemize}
...
\begin{enumerate}
    \item Market forces (3rd party effects). State that the supply and demand of markets are enough to regulate the safety of the products on the market. Companies may not sell safe products and may create commetcials to make people buy them even thought they are not safe.
    \item Tort and insurance: inefficent and retrospective. Companies provide financial compensation for victims of unsafe products. Companies may spend a big amount of money.
    \item State regulation: risk based, can be bureaucratic. Most of the countries don't rely on market forces or tort and insurance, so they create organizations who regulate the standards for safety products. Maintain these organizations might be burocratic and expensive.
\end{enumerate}
...
Weaknesses:
\begin{itemize}
    \item poorly documented processes
    \item ad hoc reviews of output
    \item low priority for safety
    \item poor resolution of conflict
    \item (unpredictable) budgetary pressures
\end{itemize}
...
\begin{itemize}
    ...
    \item Reduce Risks
    \item Reduce unnacceptable Risks and Demonstrate Reductions
    \item Implics High Level of Documentation
\end{itemize}

\textbf{Equipment Under Control (EUC):} Equipment, machinery, apparatus or plant used for manufacturing, process, transportation, medical or other activities. Software is not hazard but hardware is.

\textbf{Safety-integrity:} probability of a safety-related system satisfactorily performing the required safety functions under all the stated conditions wihtin a stated period of time.

\textbf{Safety integrity level:} discrete level for specifying the safety integrity requirements... where SIL 4 has the highest level of safety integrity and SIL 1 the lowest. SIL is a measurement of performance required for a safety instrumented function (SIF).

\begin{center}
    \large{\textbf{Risk = frequency $\times$ cost}}
    How to calculate risk
\end{center}

\section{Main components of Risk Analysis}

\begin{itemize}
    \item Hazard Identification
    \item Probability and Consequence Assessment
    \item Acceptability Assessment
    \item Mitigation
\end{itemize}

\subsection{Hazard Identification}

\textbf{Event Frecuency.} There are several categories in the frecuency: Frequent, probable, occasional, remote, improbable, incredible.

\textbf{Hazard Consequence.} Different categories: catstrophic, critical, marginal, negligible.

\begin{itemize}
    \item Estimate EUC risk of all hazards
    \item Quantitative or qualitative techniques
    \item Must be documented and maintained
    \item User must choose the method
\end{itemize}


\chapter{Standard 26262}
...
\chapter{Fault Trees Analysis (FTA)}
...
\chapter{Faulire Modes and Effects Analysis (FMEA/FMECA)}
...
\section{Hazard and operability study (HAZOPS)}
